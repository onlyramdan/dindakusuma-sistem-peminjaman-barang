...
import { deleteRooms } from "@/helpers/dashboard/rooms/delete-rooms";
...
const DeleteButton = ({ id, name, setOpen }: DeleteProps) => {
  const { isSuccess, mutate, isLoading, isError } = deleteRooms();
  ...
  useEffect(() => {
    if (isSuccess) {
      toast({
        ...
        description: `Ruangan \"${name}\" telah berhasil dihapus`,
      });
      setOpen(false);
    }
